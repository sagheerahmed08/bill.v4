...
const Billing = () => {
  ...
  const openSizeSelectionModal = (product) => {
    ...
    setSelectedSizeForCart(product.product_variants[0]?.size || ''); // Default to first available size
  ...
  const handleSizeSelectedAndAddToCart = () => {
    ...
    if (existingItem) {
      ...
      setCart([...cart, { 
        ...
        id: productForSizeSelection.id, // Ensure product ID is used
        cartKey: cartItemKey, 
        size: selectedSizeForCart, 
        stock: variant.stock, // Stock for this specific variant
        price: productForSizeSelection.price, // Use base product price
        quantity: 1 
      }]);
    }
